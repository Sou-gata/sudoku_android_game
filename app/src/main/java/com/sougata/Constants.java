package com.sougata;

public final class Constants {
    public static final int UNASSIGNED = 0;
    public static final int GRID_SIZE = 9;
    public static final int BOX_SIZE = 3;
    public static final int[] NUMBERS = {1, 2, 3, 4, 5, 6, 7, 8, 9};
    public static final String[] LEVEL_NAME = {"Easy", "Medium", "Hard", "Expert", "Nightmare"};
    public static final int[] LEVEL = {29, 38, 47, 56, 65};
    public static final String SAVE_LOCATION = "Pictures/sudoku";
    public static final String[] TYPES = {"match", "daily", "event"};
    public static final int SPLASH_DELAY = 2750;
    public static final int ALLOWED_MISTAKES = 3;
    public  static final String[] MONTHS = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
    public static final String[] FULL_MONTHS = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
    public static final String SHARED_PREF_NAME = "AppSettings";
    public static final boolean SOUND = true;
    public static final boolean VIBRATE = true;
    public static final boolean REMOVE_NOTES = true;
    public static final boolean HIGHLIGHT_NUMBERS = true;
    public static final boolean HIGHLIGHT_REGION = true;
    public static final boolean ADVANCE_NOTE = true;
    public static final boolean HIGHLIGHT_NOTES = true;
    public static final boolean DARK_MODE = false;
    public static final int HINTS_PER_LEVEL = 1;
    public static final float ADVANCE_NOTE_PER_LEVEL = 0.25f;
}
